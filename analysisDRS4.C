...
void analyze(const char* path, int ch1, int ch2, int ch3, int ch4, double tscale = 1.0)
{
    TString file_name;
    ...
    float w1f[npoints], w2f[npoints], w3f[npoints], w4f[npoints];
    float t1f[npoints], t2f[npoints], t3f[npoints], t4f[npoints];
 ...
 //       loadTree->SetBranchAddress("t1",&t3f, &b_t3f);
    }
    //________________________________________________________
    ...
    //    loadTree->SetBranchAddress("t1",&t4f, &b_t4f);

    }
    //________________________________________________________
    
    /* std::cout << "base_line section--------------------" << endl;
     
     for (unsigned long int base = 0; base < Entries; base++)
	    {
     b_w2f->GetEntry(base);
     b_w3f->GetEntry(base);
     for(int b_fill = 0; b_fill < m_inoise; b_fill++)
     {
     if(ch2 == 2) b2.push_back(w2f[b_fill]);
     if(ch3 == 3) b3.push_back(w3f[b_fill]);
     }
     
     }
     
     pulse_baseline(b2, baseline2, noise2);
     double base2 = std::get<0>(baseline2);
     pulse_baseline(b3, baseline3, noise3);
     double base3 = std::get<0>(baseline3);
    ...
    for(int fil = 0; fil < Entries; fil++)
    {
    
        loadTree->GetEntry(fil);
  
         if (fil%100==0)
             cout << "Processing event  " << fil << endl;
        ...
        for(int i = 0; i < npoints; i++)
        {
            ...
            if(ch2 == 2){w2.push_back( a2); t2.push_back(  t1f[i] ); wbase2.push_back( a2);}
            if(ch3 == 3){w3.push_back( a3 ); t3.push_back(  t1f[i] ); wbase3.push_back( a3);}
            if(ch4 == 4){w4.push_back( a4*-1.0); t4.push_back(  t1f[i] ); wbase4.push_back( a4*-1.0 );}
        }
        ...
        if(ch1 == 1)
        {
          ...
          //  for(int i = 0; i < points ; i++){w1[i] = -1.0*w1[i]; wbase1[i] = -1.0*wbase1[i];} //invert pulses
            
            //pulse_baseline(w1, baseline1, noise1);
            
            //rms1.push_back( noise1.first );
            
            //for(int i = 0; i < points ; i++){w1[i] = w1[i] - baseline1.first;}
            ...
            //for(int i = 0; i < points; i++){w1[i] = -1.0*w1[i]; wbase1[i] = -1.0*wbase1[i];}
        }
        ...
        if(ch2 == 2)
        {
          ...
          //  for(int i = 0; i < points ; i++){w2[i] = -1.0*w2[i]; wbase2[i] = -1.0*wbase2[i];}
            
            //pulse_baseline(w2, baseline2, noise2);
            
            //rms2.push_back( noise2.first );
            
            //for(int i = 0; i < points ; i++){w2[i] = w2[i] - baseline2.first;}
            
            pmax2.push_back( pulse_max_fit(points, w2, t2,0.20,0.85, m_inoise));
            ...
            rise2_1090.push_back(rise_fit(points, w4, t4,0.10,0.9, m_inoise ) );
      ...
      //      for(int i = 0; i < points; i++){w2[i] = -1.0*w2[i]; wbase2[i] = -1.0*wbase2[i];}
       ...
       //     for(int i = 0; i < points; i++){w3[i] = -1.0*w3[i]; wbase3[i]=-1.0*wbase3[i];}
            
            //pulse_baseline(w3, baseline3, noise3);
            
            //rms3.push_back( noise3.first );
            
            //for(int i = 0; i < points ; i++){w3[i] = w3[i] - baseline3.first;}
        ...
        if(ch4 == 4)
        {
         ...
         //   for(int i = 0; i < points; i++){w4[i] = -1.0*w4[i]; wbase4[i]=-1.0*wbase4[i];}
            ...
            //pulse_baseline(w4, baseline4, noise4);
            
            //rms4.push_back( noise4.first );
            
            //for(int i = 0; i < points ; i++){w4[i] = w4[i] - baseline4.first;}
            
            pmax4.push_back( pulse_max_fit(points, w4, t4,0.20,0.85, m_inoise) );
            ...
            rise4_1090.push_back( rise_fit(points, w4, t4,0.10,0.9, m_inoise ) );
