// Andriy Zatserklyaniy, April 24, 2014
...
struct OscTimeHeader {
   UChar_t TIME[8];                 // char word T I M E
   UChar_t BNo[2];                  // B #
...
void osctree(const char* ifname)
{
 ...
 float max =0;
   ...
   // read the time header
   ifile.read(oscTimeHeaderRecord.buffer, sizeof(OscTimeHeader));
    cout<<sizeof(OscTimeHeader)<<endl;
    cout<< oscTimeHeaderRecord.buffer <<endl;
   const Char_t header_time[] = {'D','R','S','2','T', 'I', 'M', 'E'};
    for (int ichar=0; ichar<8; ++ichar) {
        cout << oscTimeHeaderRecord.timeHeader.TIME[ichar]<<endl;
        cout<< header_time[ichar]<<endl;
    assert(oscTimeHeaderRecord.timeHeader.TIME[ichar] == header_time[ichar]);
    }
   ...
   for (int ichannel=0; ichannel<4; ++ichannel) {
      ...
      bool found_channel = false;
      for (int iheader_channel=0; iheader_channel<4; ++iheader_channel) {
         // std::cout<<  header_channel[iheader_channel]<< std::endl;
         // std::cout<<(const char*)oscTimeRecord[ichannel].oscTime.C001<<endl;
         if (std::strncmp((const char*)oscTimeRecord[ichannel].oscTime.C001, header_channel[iheader_channel],4) == 0) {
            found_channel = true;
            channels.push_back(iheader_channel);
            break;
   ...
   for (unsigned ich=0; ich<channels.size(); ++ich) {
      switch (channels[ich]) {
         ...
         case 1: tree->Branch("c2", &y[0], "c2[1024]/F");   break;
         case 2: tree->Branch("c3", &y[3], "c3[1024]/F");   break;
         case 3: tree->Branch("c4", &y[1], "c4[1024]/F");   break;
      }
   }
   ...
   while(ifile.read(oscEventVoltageRecord.buffer, oscEventVoltageSize))
   {
      ...
      for (unsigned ichannel=0; ichannel<channels.size(); ++ichannel) {
         ...
         for (int i=0; i<1024; ++i) {
            Int_t curr = (tcell + i) % 1024;
            tsum += oscTimeRecord[ichannel].oscTime.t[curr];
            x[ichannel][i] = tsum;
             y[ichannel][i] = (oscEventVoltageRecord.oscEventVoltage.oscVoltage[ichannel].voltage[i] - 65535./2)*1000./65535.0;    // mV
             if  (y[ichannel][i] >1e3 ||  y[ichannel][i] <-1e3)  y[ichannel][i] = 10.0;
             if (ichannel==2&&(j<=2||j>998) ){cout<<x[ichannel][i]<<","<<y[ichannel][i]<<endl;}
         }
      }
      tree->Fill();
      j++;
   ...
   // g0->Draw("ap");
    ...
    new TCanvas;
    tree->Draw(Form("c%d:t1",channels[channels.size()-1]+1), "Entry$<100");
