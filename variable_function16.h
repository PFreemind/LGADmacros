...
int getTrigID(vector<double>  trig, vector<double>  LSBv, double level, double period = 25.0, double tstep = 0.2, double tOff = 10.0, double tOff1 = 14.0){
   ...
   //check value after 12.5 ns
    ...
    double sum =0;
    double count =0;
    //find the first leading edge to calcuate offset
    int iOff = 0;
    for (int i{0}; i<trig.size()-1; i++ ){
        if (trig.at(i)*-1.0>0.5*level && trig.at(i+1)*-1.0<0.5*level ){
            iOff= i;
            break;
        }
    }
    ...
    int bytes[16];
    for (int i{0}; i<7;i++){
        center = int(tOff/tstep + double(i)*period/tstep +iOff);
       // cout<<center*tstep<<endl;
        ...
        }else {bit = round( (LSBv.at(LSBv.size()-1) )*-1.0 /level/1.0);}
        sum=0;
        count=0;
        bytes[i]=bit;
       // cout<<bit;
  ...
  //  for(int i{0};i<7;i++) cout <<bytes[i];
